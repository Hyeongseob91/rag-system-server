...
async def query(
    ...
    logger.info("[Query] 요청 수신: %s (%s)", request.question[:50], user_info)
    start = time.time()
    ...
    try:
        ...
        if current_user:
            try:
                ...
                logger.debug("[Query] 대화 기록 DB 저장 완료")
            except Exception as e:
                # DB 저장 실패해도 응답은 반환
                logger.warning("[Query] DB 저장 실패: %s", str(e))

        logger.info("[Query] 응답 완료: %.1fms, 출처=%d개",
                   processing_time_ms, len(sources))
        ...
        logger.error("[Query] 처리 실패: %s", str(e), exc_info=True)
